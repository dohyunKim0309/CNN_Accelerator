`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// Module Name: conv2_winograd_engine   ★★ M-PIPE VARIANT (파일=_mpipe.v) ★★
//   ▷ 원본 conv2_winograd_engine.v 의 drop-in 대체본. 모듈명 동일 → Vivado 에서
//     원본 .v 를 sources 에서 빼고 이 파일을 넣어 교체(둘 다 넣으면 중복정의 에러).
//   ▷ 변경점: M-class(gpim_q→m_im_flat = 25b 가산 + conj 부호반전, routed logic-bound)
//     200MHz 타깃 — msum=acc+gpre_q 가산기와 wino_m_assemble 사이에 register 1단 삽입.
//     m_valid = issue+11→+12 (latency +1 cycle, throughput 불변). tag(15→16)/drain(8→9)
//     동반. c2pool 출력 bit-exact 유지(iverilog 검증). 코드내 ★Mpipe 주석 참조.
// Description:
...
module conv2_winograd_engine (
    ...
    reg [3:0] mdrain_cnt;
    // drain window: ★Mpipe 로 m latch = issue+12 (원본 +11). 마지막 issue T_L 의 m latch
    //   = T_L+12 까지 mul_en_q4 필요 = mul_en@(T_L+8). mdrain<9 → mul_en T_L+1..T_L+9 (마진 1).
    wire mul_en  = compute_active || (state==DRAIN && mdrain_cnt < 4'd9);   // ★Mpipe: 8→9
    ...
    // ★Mpipe: msum 파이프 레지스터 1단 — [acc+gpre_q 가산기] | [m_assemble conj-부호반전] 분리.
    //   원본은 gpre_q → adder → m_assemble(negate) → m latch 가 한 cycle(25b 캐리체인 2개
    //   직렬 = routed M-class logic-bound 58%). 여기를 reg 로 끊음. msum_*_q 는 free-run
    //   (mul_en_q4) → grp1 cycle 에 acc(grp0)+gpre_q(grp1) 캡처 → 다음 cycle 에 m latch.
    reg signed [MW-1:0] msum_re_q [0:25];
    reg signed [MW-1:0] msum_im_q [0:25];
    wire [26*MW-1:0] msum_q_re_flat, msum_q_im_flat;
    generate for (gk = 0; gk < 26; gk = gk + 1) begin : g_msumq
        assign msum_q_re_flat[gk*MW +: MW] = msum_re_q[gk];
        assign msum_q_im_flat[gk*MW +: MW] = msum_im_q[gk];
    end endgenerate

    wire [36*MW-1:0] asm_re_flat, asm_im_flat;
    wino_m_assemble #(.MW(MW)) u_asm (
        .sre_flat(msum_q_re_flat), .sim_flat(msum_q_im_flat),   // ★Mpipe: registered msum
        .mre_flat(asm_re_flat),  .mim_flat(asm_im_flat)
    );

    reg [6:0]        grp_pipe, vld_pipe;   // ★Mpipe: 6→7bit (grp0 acc=[5], grp1 m latch=[6])
    reg [36*MW-1:0]  m_re_flat, m_im_flat;
    reg              m_valid;
    integer kk;
    always @(posedge clk) begin
        if (rst) begin
            grp_pipe<=7'b0; vld_pipe<=7'b0; m_valid<=1'b0;
            ...
            for (kk=0; kk<26; kk=kk+1) begin
                ...
                msum_re_q[kk]<={MW{1'b0}}; msum_im_q[kk]<={MW{1'b0}};   // ★Mpipe
            end
        end else if (mul_en_q4) begin
            grp_pipe <= {grp_pipe[5:0], mul_grp_q4};   // ★Mpipe: 7bit shift
            vld_pipe <= {vld_pipe[5:0], 1'b1};
            for (kk=0; kk<26; kk=kk+1) begin   // ★ C-1: cross-lane 합 register
                gpre_q[kk] <= gpre[kk];
                gpim_q[kk] <= gpim[kk];
            end
            // ★Mpipe: msum free-run register (acc+gpre_q). grp1 cycle 값을 다음 cycle m latch 가 사용.
            for (kk=0; kk<26; kk=kk+1) begin
                msum_re_q[kk] <= $signed(msum_re_flat[kk*MW +: MW]);
                msum_im_q[kk] <= $signed(msum_im_flat[kk*MW +: MW]);
            end
            // grp0 acc load : [5] (원본과 동일 타이밍, m_valid 와 분리)
            if (vld_pipe[5] && grp_pipe[5] == 1'b0) begin
                for (kk=0; kk<26; kk=kk+1) begin   // grp0 : 첫 4 IC partial 적재
                    acc_re[kk] <= gpre_q[kk];
                    acc_im[kk] <= gpim_q[kk];
                end
            end
            // ★Mpipe: grp1 M latch : [6] (acc load 보다 1 cycle 늦음 = msum_q 경유)
            if (vld_pipe[6] && grp_pipe[6] == 1'b1) begin   // grp1 : 8-IC 완성 M latch
    ...
    //==========================================================================
    // tag pipeline (oc/trow/tcol) — ★Mpipe: issue→m_valid = +12 (원본 +11, msum reg +1)
    //   = 입력측 +4 + DSP 3 + pre_q + gp쌍 + gpre_q + ★msum_q + m latch.
    //   출력변환 4-stage(+4), trunc +1, cwe +1 → collector 가 tg_*[16] 사용.
    //==========================================================================
    wire [3:0] issue_oc   = compute_cnt[4:1];
    reg  [3:0] tg_oc   [1:16];   // ★Mpipe: 15→16
    reg  [2:0] tg_trow [1:16];
    reg  [2:0] tg_tcol [1:16];
    integer ti;
    always @(posedge clk) begin
        if (rst) for (ti=1; ti<=16; ti=ti+1) begin tg_oc[ti]<=0; tg_trow[ti]<=0; tg_tcol[ti]<=0; end
        else begin
            tg_oc[1]<=issue_oc; tg_trow[1]<=trow_cnt; tg_tcol[1]<=tile_cnt;
            for (ti=2; ti<=16; ti=ti+1) begin
    ...
    //   ★Mpipe: coc/tag 는 tg_*[16] (issue+12 M + OT+4 → cwe set cycle 에 [16] = issue context).
    //==========================================================================
    ...
    always @(posedge clk) begin
        if (rst) begin cwe<=0; coc<=0; coc_trow<=0; coc_tcol<=0; tile_done<=0; done_trow<=0; done_tcol<=0; end
            ...
            cwe<=ot_valid; coc<=tg_oc[16]; coc_trow<=tg_trow[16]; coc_tcol<=tg_tcol[16];   // ★Mpipe: [15]→[16]
            tile_done<=1'b0;
            if (cwe) begin
