...
이 문서는 `conv2_engine` 의 cycle 별 동작을 기록한 single source of truth.
...
**표의 모든 셀은 "해당 cycle 시작 시점에서의 register 값"** (= 직전 edge 에서 latch 된 값) 을 나타낸다. 조합 신호 (`shift_en`, BRAM addr, `row2_in`, `dout` 등) 는 그 cycle 의 register 값으로 즉시 계산되는 값.

본 doc 의 cycle 0 = **PIPELINE_FILL 첫 cycle**. testbench 측정 cycle (reset 해제 기준, prelude 포함) 과의 매핑은 **§8** 참조. 빠른 훑어보기 / quick reference 컴팩트 표는 **§11 부록** 참조.
...
- Testbench 가 측정하는 cycle 번호 (reset 해제 기준, IDLE / LOAD_WEIGHTS / DONE prelude 포함) 와의 매핑은 **§8** 참조.
...
**일반화 invariant (L=2 + steady-state)**:
...
**Verification anchor**: 첫 c2pool write 의 mem 갱신 = 첫 PE input cycle 57 + 14 = cycle 71. (2026-05-28 testbench 로 정합 확정.)

### 5.2 Adder en 5-cycle window — 마지막 픽셀의 정확성

`krow_ic_adder_tree` 는 **5-stage pipeline 이며 각 stage 가 `en` 으로 게이팅**된다. 즉 마지막 valid PE 출력이 sum register 까지 propagate 하려면 `en=1` 이 **5 cycle 연속 유지**되어야 한다.
...
`pe_en_pipe[3]` 하나만 게이팅하면 한 image 의 마지막 PE 출력 (cycle 1784 의 (23, 23) K_col=2) 이후 1 cycle 만에 `en=0` → s1 에서만 latch 되고 s2~s5 가 stuck. `kcol_accumulator` 가 stale sum 을 누적해 **마지막 2 픽셀 (23, 22), (23, 23) 의 K_col=2 contribution 이 누락**된다.

이 bug 는 corner pixel 입력이 모두 0 (background) 인 image 에서는 mask 되어 invisible (single-image image 0 검증 통과). multi-image 검증 중 image 28 에서 catch. 상세 분석 (mismatch 데이터, mechanism trace, kcol_acc 시점 별 입력) 은 **`conv2_adder_drain_bug_fix.md`** 참조.

영향 요약:

- **Cycle count**: 변화 없음 (1796/image 그대로, wdone @ cycle 1796 그대로).
- **Steady state**: `pe_en_pipe[3..7]` 모두 1 → OR 결과 1, 기존 동작과 차이 없음.
- **DRAIN 중**: 추가 5 cycle 동안 adder pipeline 이 propagate 후 자연스럽게 0. `kcol_en=0` / `truncate_en=0` 이라 downstream 무해.
- **합성 자원**: OR 4 LUT 정도. 무시 가능.

**일반 원칙**: pipelined 모듈의 `en` 게이팅은 "pipeline depth 만큼의 window" 로 정의. 모든 pipelined 모듈 (adder, FIFO, accumulator 등) 에 동일하게 적용.
...
| A2 | 57 | 첫 COMPUTE_HOLD cycle | state=HOLD, counter (2, 5), kw_cnt=0, shift_en=0, win_r2=[(2,0),(2,1),(2,2)], PE in col=((0,0),(1,0),(2,0)) |
| A3 | 60 | 출력 (0, 1) HOLD0 | win_r2[2] = (2, 3) (= col_pos_2 row 2 of output (0, 1)) |
| A4 | 125 | WRAP entry trigger | state=ADV, counter (3, 1), col_cnt==1 → state next-edge ← WRAP |
| A5 | 129 | WRAP 종료 후 정렬 | state=HOLD, counter (3, 5), win_r2=[(3,0),(3,1),(3,2)] (= 출력 (1, 0) 의 row 2) |
...
### 8.1 Prelude — testbench cycle 1..582
...
### 8.2 LOAD_WEIGHTS 580 cycle sub-breakdown
...
### 8.3 wdone fire 시점 (datapath, `conv2_engine.v` §14)
...
- **line_buffer 내부 ptr 갱신 순서**: §1.2 의 lb1.dout, lb2.dout 식은 "shift_en=1 매 cycle" 가정. compute phase 에서 shift_en 은 ADV 에만 1 → effective delay 가 26 ADV events = 78 cycle. 행 경계 (WRAP) 와 cap 의 영향은 §3, §4 의 trace 와 외부 동작상 일치 (multi-image PASS 로 검증) 하나, lb 의 ptr 위치까지 cycle 별로 직접 trace 하지는 않음. 단위 검증 단계에서 lb 내부 mem dump 로 추가 검증 권장.
...
| 2026-05-28 (a) | single-image testbench PASS (compute total 2378 cycle, mismatches 0/576) 와 정합 확인. §4.1 의 마지막 DRAIN 구간을 mem update edge (1795→1796) 와 wdone fire (cycle 1796) 분리 — 기존 `edge 1796→1797 mem 갱신` 표기는 §0.4 / §5.1 의 lag-12 정의와 1 cycle 어긋남. §7 anchors A9/A10/A11 로 분리. §8 testbench cycle 매핑 추가. |
| 2026-05-28 (b) | multi-image testbench (image 0..99) 에서 발견된 adder drain bug fix 반영 — `krow_ic_adder_tree` 의 5-stage pipeline 에 맞춰 `adder_en` 을 5-cycle OR window (`pe_en_pipe[3..7]`) 로 확장. §0.4 footnote + §5.2 신규 단락 추가. Cycle count 영향 없음. 상세 `conv2_adder_drain_bug_fix.md`. |
| 2026-05-28 (c) | `conv2_timing_tables.md` 와 통합 — tables.md 의 single-row 컴팩트 표를 §11 부록으로 흡수. tables.md 파일 삭제. open items 의 §5 PE→c2pool lag / DRAIN cycle 두 항목은 testbench PASS 로 resolved 되어 본문에서 삭제 (본 변경 이력에만 기록). |

---

## 11. 부록 — Quick reference (compact tables)

본 doc 의 §1~§4 와 동일한 cycle 범위를 single-row 표 형식으로 압축. 빠른 훑어보기 / cross-check 용. 분석/derivation 은 본문 §1~§4 참조.

### 11.0 부록 표의 약속 (요약)

- 셀 값 = **cycle 시작 시점의 register 값** (직전 edge 에서 latch).
- `phase` 컬럼: HOLD/ADV → `kw=N` (N=0/1/2), WRAP → `w=N`, DRAIN → `d=N` (N=0..11), FILL / IDLE / LOAD / DONE → `—`.
...
### 11.1 PIPELINE_FILL → 첫 compute (cycle 54..62)
...
### 11.2 Steady-state 1 픽셀 (r, c) — T0 = HOLD0 of (r, c), c = 0..20
...
### 11.3 Row boundary WRAP for output (0, 23) — cycle 122..132
...
### 11.4 마지막 행 (r=23) + DRAIN — cycle 1770..1797
...
cycle 1795 (DRAIN d=10) edge → 1796: 마지막 c2pool mem[575] 갱신 + `wdone_reg ← 1` (lag-12 pipeline; §0.4 / §5.1 참조).
...
| 파일 | 역할 |
|---|---|
| `conv2_design.md` | 아키텍처 / 모듈 계층 / 핵심 설계 결정 |
| `conv2_adder_drain_bug_fix.md` | adder_en 5-cycle window bug 의 상세 분석 (image 28 mismatch) |
| `../../CONV2_TIMING_FIX.md` | c1c2 BRAM L=1 → L=2 fix 의 cycle-by-cycle 증명 |
| `../../docs/DSP48E1_signed8x8_SIMD_Packing.md` | SIMD packing 알고리즘 |
| `conv2_engine.v` | top-level RTL (datapath + delay pipeline + c2pool write / handshake) |
| `conv2_fsm.v` | FSM RTL (8 state, counters, handshake) |
| `scripts/golden_sim/0_reference.py` | Python reference (expected output) |
| `scripts/header_hex_gen/conv2_simd_pack.py` | weight pre-pack (offline) |
