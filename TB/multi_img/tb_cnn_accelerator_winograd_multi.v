`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// tb_cnn_accelerator_winograd_multi.v
//   ★ WINOGRAD 빌드 변형 (cnn_accelerator_winograd.v) 전체 파이프라인 검증 TB.
//     Conv1 = conv1_2x_engine, Conv2 = conv2_winograd_engine(복소수 Winograd F(4,3)).
...
//     - enable=1, start pulse → conv2 LOAD_WEIGHTS(loader) + timer
//     - per image  : Input BRAM Port A write → img_ready pulse
//     - 검증       : dut.fc.logit_reg(logit) bit-exact + bram_output readback(argmax)
//
//   Overlap (input_consumed backpressure):
//     dispatcher 가 input_consumed 기준(적재 - consumed < 2, input BRAM 2-bank)으로
//     다음 image 를 미리 적재 → pipeline overlap. result_collector 가 img_done 마다 검증.
//
//   iverilog (cwd=프로젝트 루트):
//     iverilog -g2012 -o wino.vvp \
//       -y RTL/conv1_2x -y RTL/conv1 -y RTL/conv2_winograd -y RTL/maxpool -y RTL/fc -y RTL/core \
//       RTL/cnn_accelerator_winograd.v \
//       TB/models/bmg_sim_models.v TB/models/dsp48e1_model.v \
//       TB/multi_img/tb_cnn_accelerator_winograd_multi.v && vvp wino.vvp
//   Vivado : 실 BMG IP + DSP48E1(UNISIM) → TB/models/* 제외, hex 경로는 아래 절대경로.
//////////////////////////////////////////////////////////////////////////////////
...
module tb_cnn_accelerator_winograd_multi;

    parameter N_IMAGES    = 100;     // user gate (data 100 까지 지원)
    parameter ACC_W       = 24;
    ...
    initial begin : main_process
        $display("\n==========================================");
        $display("  cnn_accelerator WINOGRAD (PL core) overlap TB  (N=%0d, CHECK_LABEL=%0d)", N_IMAGES, CHECK_LABEL);
        $display("==========================================");
        ...
        load_w2();
        load_fcw();
        $display("[TB] @ cyc %0d : weights loaded (w1/winograd-U/fc)", cycle_cnt);

        // 가동 + weight load (conv2 LOAD_WEIGHTS) + timer
        @(negedge clk); enable = 1'b1;
        pulse_start();
        $display("[TB] @ cyc %0d : enable=1, start pulsed (conv2 → LOAD_WEIGHTS)", cycle_cnt);
        weight_loaded = 1'b1;
